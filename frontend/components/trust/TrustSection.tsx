  ...
  return (
    ...
    >
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        <div className="absolute -end-20 top-16 h-64 w-64 rounded-full bg-nuria-soft/30 blur-3xl" />
        <div className="absolute -start-16 bottom-12 h-56 w-56 rounded-full bg-nuria-rose-soft blur-3xl" />
      </div>
        ...
        <div className="mt-14 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:gap-8">
          {trustKeys.map((key) => {
            ...
            return (
              <article
                key={key}
                className="group rounded-3xl border border-slate-100 bg-white p-6 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-nuria/25 hover:shadow-xl sm:p-7"
              >
                <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-nuria-soft text-nuria transition-colors duration-300 group-hover:bg-nuria-soft">
